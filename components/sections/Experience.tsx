    ...
    <section id="experience" className="py-24 sm:py-32">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="relative">
          ...
          <StaggerContainer className="flex flex-col gap-6 sm:pl-16" staggerDelay={0.15}>
            {experience.map((job) => (
              <article
              ...
              >
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-2xl"
                  style={{ background: "radial-gradient(600px at 50% 0%, rgba(99,102,241,0.04), transparent 70%)" }}
                />
                ...
                <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-6 mb-6">
                  ...
                  <div className="flex-shrink-0 text-right">
                    <div className="text-sm text-white/40 font-mono">{job.period}</div>
                    <div className="text-xs text-white/30 mt-1">{job.location}</div>
                  </div>
                </div>
        ...
        <FadeIn delay={0.3} className="mt-12">
          <div className="glass border border-white/[0.07] rounded-2xl p-6 sm:p-8">
            <div className="text-xs text-white/30 font-mono uppercase tracking-widest mb-4">Education</div>
            {education.map((edu) => (
              <div key={edu.degree} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                  <h3 className="text-white font-semibold">{edu.degree}</h3>
                  <p className="text-white/50 text-sm">{edu.institution}</p>
                </div>
                <div className="text-white/30 font-mono text-sm">{edu.year}</div>
              </div>
            ))}
          </div>
        </FadeIn>
